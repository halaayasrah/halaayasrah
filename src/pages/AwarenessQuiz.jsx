...
const QUESTIONS = [
  {
    ...
    answer: 1,
    explanation:
      'Phishing involves deceptive emails, messages, or websites that trick users into revealing passwords, credit card numbers, or other sensitive data.',
  },
  {
    ...
    answer: 2,
    explanation:
      'Strong passwords combine uppercase, lowercase, numbers, and special characters and avoid common dictionary words.',
  },
  {
    ...
    answer: 1,
    explanation:
      '2FA adds a second verification step (like an SMS code or authenticator app) beyond just a password, making accounts much harder to compromise.',
  },
  {
    ...
    answer: 2,
    explanation:
      'Never click suspicious links. Report phishing attempts to your IT security team so they can investigate and protect the organization.',
  },
  {
    ...
    answer: 1,
    explanation:
      "Ransomware encrypts victims' files and demands a ransom for the decryption key, causing massive disruption to individuals and organizations.",
  },
]
...
export default function AwarenessQuiz() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center">
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(232,93,117,0.2), rgba(232,93,117,0.05))',
          }}
        >
        ...
        <p className="text-gray-400 max-w-md mb-8">
          Test your cybersecurity knowledge with {QUESTIONS.length} questions covering phishing,
          passwords, 2FA, malware, and more.
        </p>
        ...
        <button
          ...
          className="btn-gradient px-10 py-4 rounded-2xl text-white font-semibold text-lg"
        >
          Start Quiz
        </button>
      </div>
  ...
  if (finished) {
    ...
    const gradeLabel = pct >= 80 ? 'Excellent' : pct >= 60 ? 'Good' : pct >= 40 ? 'Fair' : 'Needs Improvement'
    const gradeColor =
      pct >= 80 ? 'text-green-400' : pct >= 60 ? 'text-[#e85d75]' : pct >= 40 ? 'text-yellow-400' : 'text-red-400'

    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center">
        <div className="card p-10 max-w-md w-full">
          <div className="text-6xl font-black mb-2" style={{ color: '#e85d75' }}>
            {pct}%
          </div>
          <h3 className={`text-2xl font-bold mb-1 ${gradeColor}`}>{gradeLabel}</h3>
  ...
  const handleNext = () => {
    ...
    if (current + 1 >= QUESTIONS.length) {
      ...
      const grade = calcGrade(pct)
      api
        .saveResult({ score: pct, total: QUESTIONS.length, topic: 'Security Awareness', grade })
        .catch(console.error)
      setFinished(true)
    } else {
  ...
  return (
    <div className="max-w-xl mx-auto py-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[#e85d75] text-sm font-medium">
          ...
          {q.options.map((opt, i) => {
            ...
            if (revealed) {
              if (i === q.answer) cls = 'border border-green-500 bg-green-500/10 text-green-300'
                ...
                cls = 'border border-red-500 bg-red-500/10 text-red-300'
            } else if (selected === i) {
      ...
      <button
        ...
        className={`w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all ${
          revealed ? 'btn-gradient text-white' : 'bg-[#2a1820] text-gray-600 cursor-not-allowed'
        }`}
      >
